...
def preview_dataset(
    ...
    *,
    ...
    mask_ops_requested = any(
        (op or {}).get("kind") in {"sam_segment", "clipseg_segment", "embed_masks"} for op in ops
    )
    mask_embeddings_requested = any((op or {}).get("kind") == "embed_masks" for op in ops)
    mask_feature_entries: List[Dict[str, Any]] = []
    mask_umap_coords: List[Optional[np.ndarray]] = []
    ...
    for index, original in enumerate(records):
        ...
        record["__run_dir__"] = run_dir
        record["__record_index__"] = index
        ...
        for op in ops:
            ...
            if dataset_kind == "csv" and kind == "summarize":
                ...
                try:
                    ...
                    logger.exception("%s failed for image '%s'", kind, record.get("image_path"))
                    ...
                    mask_payload = op_result.get("masks") if isinstance(op_result, dict) else []
                    if not isinstance(mask_payload, list):
                        mask_payload = []
                    record["masks"] = mask_payload
                    if op_result:
                        provenance.record_operation(record_entry, kind, params, op_result)
                if "masks" not in base_columns and "masks" not in new_columns_seen:
                    schema_new_columns.append("masks")
                    new_columns_seen.add("masks")
            elif dataset_kind == "images" and kind == "embed_masks":
                mask_payload = record.get("masks")
                if not isinstance(mask_payload, list) or not mask_payload:
                    message = "embed_masks requires segmentation masks"
                    record_errors.append(message)
                    provenance.log(
                        "ERROR",
                        message,
                    ...
                    record["mask_features"] = []
                    continue
                try:
                    op_result = run_operation(record, kind, params)
                except Exception as exc:  # pragma: no cover - defensive
                    logger.exception("embed_masks failed for image '%s'", record.get("image_path"))
                    ...
                    record["mask_features"] = []
                    continue
                features_raw = op_result.get("mask_features") if isinstance(op_result, dict) else []
                ...
                record["mask_features"] = features_raw
                for feature_index, feature in enumerate(features_raw):
                    if not isinstance(feature, dict):
                        continue
                    vector_array = _vector_from_output(feature.get("embedding"))
                    mask_dict = mask_lookup.get(str(feature.get("id")))
                    if mask_dict is not None:
                        if feature.get("embedding") is not None:
                            mask_dict["embedding"] = feature.get("embedding")
                        if feature.get("umap") is not None:
                            mask_dict["umap"] = feature.get("umap")
                        if feature.get("cluster") is not None:
                            mask_dict["cluster"] = feature.get("cluster")
                    if vector_array is None:
                        continue
                    mask_feature_entries.append(
                        {
                            "record_index": index,
                            "mask_index": feature_index,
                            "feature": feature,
                ...
                if op_result:
                    provenance.record_operation(record_entry, kind, params, op_result)
                if "mask_features" not in base_columns and "mask_features" not in new_columns_seen:
                    schema_new_columns.append("mask_features")
                    new_columns_seen.add("mask_features")

        if record_errors:
            for err in record_errors:
                provenance.log("WARNING", err, record_entry=record_entry)

        record.pop("__run_dir__", None)
        record.pop("__record_index__", None)
    ...
    for op in ops:
        ...
        if kind == "umap":
            ...
            output_field = str(
                ...
                or ("mask_umap" if source_field == "mask_embedding" else "umap")
            )

            if source_field == "mask_embedding":
                if not mask_feature_entries:
                    message = "umap requires mask embeddings"
                    provenance.log(
                        "WARNING",
                        message,
                ...
                coords_holder: List[Optional[np.ndarray]] = [None] * len(mask_feature_entries)
                valid = [
                    (idx, entry["embedding"])
                    for idx, entry in enumerate(mask_feature_entries)
                    if entry.get("embedding") is not None
                ]
                if len(valid) >= 2:
                    matrix = np.vstack([embed for _, embed in valid])
                    ...
                    for (entry_idx, _), coord in zip(valid, coords_valid):
                        coords_holder[entry_idx] = coord.astype(np.float32)
                        feature = mask_feature_entries[entry_idx]["feature"]
                        feature["umap"] = [float(coord[0]), float(coord[1])]
                        mask_dict = mask_feature_entries[entry_idx].get("mask_dict")
                        if isinstance(mask_dict, dict):
                            mask_dict["umap"] = feature["umap"]
                elif len(valid) == 1:
                    idx, _ = valid[0]
                    coords_holder[idx] = np.array([0.0, 0.0], dtype=np.float32)
                    feature = mask_feature_entries[idx]["feature"]
                    feature["umap"] = [0.0, 0.0]
                    mask_dict = mask_feature_entries[idx].get("mask_dict")
                    if isinstance(mask_dict, dict):
                        mask_dict["umap"] = feature["umap"]
                mask_umap_coords = coords_holder
                umap_results[output_field] = mask_umap_coords
                if output_field != "mask_umap":
                    umap_results.setdefault("mask_umap", mask_umap_coords)
                if any(coord is not None for coord in mask_umap_coords):
                    coords_array = _coords_array(mask_umap_coords)
            ...
            if source_field == "umap" and "umap" not in umap_results and mask_umap_coords:
                source_field = "mask_umap"

            if source_field == "mask_umap":
                if not mask_umap_coords:
                    provenance.log(
                        "WARNING",
                        "hdbscan requires mask UMAP coordinates",
                        context={"source": source_field},
                    )
                    continue
                labels = [-1] * len(mask_feature_entries)
                valid = [
                    (idx, coord) for idx, coord in enumerate(mask_umap_coords) if coord is not None
                ]
                if valid:
                    ...
                    for (entry_idx, _), label in zip(valid, labels_valid):
                        labels[entry_idx] = int(label)
                        feature = mask_feature_entries[entry_idx]["feature"]
                        feature["cluster"] = int(label)
                        mask_dict = mask_feature_entries[entry_idx].get("mask_dict")
                        if isinstance(mask_dict, dict):
                            mask_dict["cluster"] = int(label)
                resolved_output = output_field or "mask_cluster"
                labels_array = np.asarray(labels, dtype=np.int32)
                filename = run_dir / f"{_sanitize_name(resolved_output)}_hdbscan.npy"
                np.save(filename, labels_array)
                artifacts[_artifact_key("hdbscan", resolved_output)] = str(filename)
                continue
    ...
    if mask_feature_entries:
        mask_embeddings_matrix = np.vstack(
            [entry["embedding"].astype(np.float32) for entry in mask_feature_entries]
        )
        mask_embeddings_path = run_dir / "mask_embeddings.npy"
        np.save(mask_embeddings_path, mask_embeddings_matrix.astype(np.float32))
        artifacts["mask_embeddings"] = str(mask_embeddings_path)

        metadata_records: List[Dict[str, Any]] = []
        for entry in mask_feature_entries:
            feature = entry["feature"]
            metadata_records.append(
                {key: value for key, value in feature.items() if key != "embedding"}
            )
        mask_metadata_path = run_dir / "mask_metadata.json"
        mask_metadata_path.write_text(json.dumps(metadata_records, ensure_ascii=False, indent=2))
        artifacts["mask_metadata"] = str(mask_metadata_path)
    elif mask_embeddings_requested:
        mask_embeddings_path = run_dir / "mask_embeddings.npy"
        mask_embeddings_path.parent.mkdir(parents=True, exist_ok=True)
        np.save(mask_embeddings_path, np.zeros((0, 0), dtype=np.float32))
        artifacts["mask_embeddings"] = str(mask_embeddings_path)
        mask_metadata_path = run_dir / "mask_metadata.json"
        mask_metadata_path.write_text(json.dumps([], ensure_ascii=False, indent=2))
        artifacts["mask_metadata"] = str(mask_metadata_path)
        provenance.log(
            "WARNING",
            "Mask embedding artifacts are empty.",
            context={"path": str(mask_embeddings_path)},
        )

    if mask_ops_requested:
        mask_root_dir = run_dir / "masks"
        mask_root_dir.mkdir(parents=True, exist_ok=True)
        artifacts["masks_dir"] = str(mask_root_dir)
        mask_pngs = list(mask_root_dir.glob("**/*.png"))
        if not mask_pngs:
            provenance.log(
                "WARNING",
                "Mask directory created but no mask images were generated.",
                context={"path": str(mask_root_dir)},
            )
